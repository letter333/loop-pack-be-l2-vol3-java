...
@Slf4j
@Component
@ConditionalOnProperty(name = "spring.batch.job.name", havingValue = "NONE", matchIfMissing = true)
public class RankingJobScheduler {
    ...
    public RankingJobScheduler(
        ...
        @Qualifier(WeeklyRankingJobConfig.JOB_NAME) Job weeklyRankingJob,
        @Qualifier(MonthlyRankingJobConfig.JOB_NAME) Job monthlyRankingJob
    ) {

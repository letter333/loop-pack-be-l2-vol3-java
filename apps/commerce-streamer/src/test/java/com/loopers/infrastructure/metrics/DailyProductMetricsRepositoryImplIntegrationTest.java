...
class DailyProductMetricsRepositoryImplIntegrationTest {
    ...
    void accumulatesLikeCountOnSameDay() {
        // Arrange & Act
        dailyProductMetricsRepository.incrementLikeCount(1L, 3);
        dailyProductMetricsRepository.incrementLikeCount(1L, 2);

        // Assert
        var entities = dailyProductMetricsJpaRepository.findAll();
        assertThat(entities).hasSize(1);
        assertThat(entities.get(0).getLikeCount()).isEqualTo(5L);
        assertThat(entities.get(0).getMetricDate()).isNotNull();
    }
    ...
    void differentProductsAreIndependent() {
        // Arrange & Act
        dailyProductMetricsRepository.incrementViewCount(1L, 10);
        dailyProductMetricsRepository.incrementViewCount(2L, 20);
    ...
    void incrementsSalesCountDaily() {
        // Arrange & Act
        dailyProductMetricsRepository.incrementSalesCount(1L, 1, 50000);
